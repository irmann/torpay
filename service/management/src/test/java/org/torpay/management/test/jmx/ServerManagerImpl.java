...
@Component
@ManagedResource(objectName = "org.torpay.management.test.jmx:name=ServerManager", description = "Server manager.")
public class ServerManagerImpl implements ServerManager {

	static final Logger LOG = LoggerFactory.getLogger(ServerManagerImpl.class);
	...
	public int changeConnectionPoolSize(int minPoolSize, int maxPoolSize) {
		Assert.isTrue((minPoolSize > 0),
				"Minimum connection pool size must be larger than zero.  min="
						+ minPoolSize);
		...
		LOG.info(
				"Changed connection pool size. min={}, max={}, current={}",
